...
> ## 適用済みの完成版があります
>
> **[GAS_コード_適用済み.gs](GAS_コード_適用済み.gs) に、A〜F をすべて当てた全文があります。**
> 下の手順を 1 つずつ当てる必要はありません。Apps Script エディタで `コード.gs` を開き、
> **全選択して、このファイルの中身に貼り替えてください**（1092 行）。
>
> 貼り替えたあとの手順は「反映後の確認」の節を見てください。
>
> 以下は「何を・なぜ変えたか」の記録です。パッチの当て方としては読まなくて構いません。

> **2026-08-04 のレビューで見つかった、当日に効く 6 件です。**
...
**あわせてフロント側を 10MB に下げます**（`index.html` の `CONFIG.VIDEO_MAX_BYTES`。適用済み）。

> **訂正: 大小関係は「フロント < サーバー」です。**
> この文書は当初「サーバー側をフロントより低くする」と書いていましたが、逆でした。
> **送信を止められるのはフロントだけ**です。サーバーは受け取ってからしか判定できないので、
> 上限を低くしても通信量は 1 バイトも減りません。つまり
> 「フロントを通ったものは必ずサーバーも通る」でなければならず、サーバー側は
> アプリを経由しない相手に対する最後の防波堤という位置づけになります。
>
> 同値（両方 10MB）にすると、`approxBytes = rawBase64.length * 0.75` が
> Base64 長から 3 の倍数へ切り上げる関係で、**ちょうど 10485760 バイトの動画だけ**
> フロントを通ってサーバーで弾かれます。2MB の余裕を持たせて避けています。
...
完成版を作る過程で、パッチ自身が持ち込んでいた後退を 4 件見つけて修正しました。
文書どおりに手で当てると、これらを踏みます。

| 内容 | 直し方 |
|---|---|
| **F の許可リストが動画を 3 種に絞りすぎ**。`.m4v` / `.3gp` / `.avi` / `.mkv` は以前は保存できていたのに、恒久エラーで拒否されるようになっていた | `video/3gpp` `video/3gpp2` `video/x-m4v` `video/x-msvideo` `video/x-matroska` `video/mpeg` を追加。動画には「動画の形式に対応していません。」を出す |
| **B のフォールバックが 30 件に激減**。フロントは 1 ページ 30 件で要求するので、`size` をそのまま渡すと 30 件しか返らない。DriveApp の順序は保証されないため「任意の 30 枚」になり、さっき撮った写真が入らない（旧版は 500 件） | 件数を `pageSize` に連動させず 200 件固定に |
| **B のフォールバック結果が正常系のキャッシュを汚染**。1 回の REST 失敗で作った劣化レスポンスが、復活後も 20 秒間すべてのゲストに配られる。↻ を押しても直らない | `albumfb_v…` の専用キーに分離。読むのもフォールバック経路だけ |
| **E の境界**（上記の訂正） | サーバーを 12MB に |

あわせて、A のコメントが「FOLDER_ID の typo を防ぐ」と読める書き方でしたが、
実際には設定ミスは `permanent` に**含めていません**。含めると、幹事が式中に設定を
直しても撮り溜めた写真が二度と送れなくなるためです。設定ミスは
デプロイ前の `verifySetup()` で潰す前提です。コメントをその旨に直しました。
